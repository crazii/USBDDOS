...
BOOL USB_InitController(uint8_t bus, uint8_t dev, uint8_t func, PCI_DEVICE* pPCIDev)
{
    ...
    _LOG("Host Controller: Bus:%d, Dev:%d, Func:%d, pi:0x%02x, IRQ: %d, INT Pin: INT%c#\n", bus, dev, func, header->PInterface, header->Device.IRQ, 'A'+header->Device.INTPIN-1);
    for(int i = 0; i < USB_MAX_HC_TYPE; ++i)
    {
        if(USBT.HC_Types[i].InitController != NULL)
        {
            ...
            PIC_MaskIRQ(header->Device.IRQ); //mask irq until controller init ready
            ...
            uint8_t iv = PIC_IRQ2VEC(header->Device.IRQ);
            _LOG("Install ISR 0x%02x\n", iv);
            DPMI_ISR_HANDLE* handle = USB_FindISRHandle(header->Device.IRQ);
            if(handle != NULL)
            {
            ...
            PIC_UnmaskIRQ(header->Device.IRQ);
            break;
        }
    }
    return OK;
...
static void USB_ConfigEndpoints(USB_Device* pDevice)
{
    ...
    for(int8_t i = 0; i < bNumEndpoints; ++i)
    {
        ...
        assert(ep);
        _LOG("Endpoint %02x created: %08lx\n", epd->bEndpointAddress, ep);
...
{
    _ASM_BEGIN //some DPMI helper save only low word part of the GPRs (pusha).
    _ASM(pushad) //just make it safe
    _ASM_END
    USB_InISR = TRUE;
