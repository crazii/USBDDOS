...
#define USBALLOC_DEBUG 0    //use DPMI_DMAMalloc directly (probably with more debug info)
...
void* USBALLOC_TransientAlloc(uint16_t size, uint16_t alignment)
{
    ...
    aptr[-1] = index;
    aptr[-2] = index != -1 ? actual : (uint16_t)(offset + 4);
    STIL();
    return aptr;
}
...
void USBALLOC_TransientFree(void* ptr)
{
    ...
    if(memory->buff.left == USBALLOC_MEMORY_SIZE)
    {
        ...
        else
        {
            ...
            {
                USBALLOC_Memory* memory = &USBALLOC_Pool[i];
                if(memory != USBALLOC_CurrentBuf && (memory->flags&(USBALLOC_FLAG_USED)) && !(memory->flags&(USBALLOC_FLAG_32)) && memory->buff.left >= 32)
                {
                    assert(memory->arena);
...
void* USBALLOC_TransientAlloc32(uint16_t sizeverify)
{
    ...
    uint32_t index = BSF(USBALLOC_Current32->mask);
